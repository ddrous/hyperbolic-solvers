...
\end{lstlisting}
Les pentes ayant été calculées en amont, l'autre étape cruciale de l'implémentation de la méthode MUSCL est l'application des pentes $s^n_{x_L}$, $s^n_{y_L}$ et $r^n_L$ susmentionnée, juste avant d'appeler la fonction de flux numérique à utiliser\footnote{Nous utiliserons le flux de Rusanov car moins précis, et on aimerait qu'il ait une précision proche de la méthode de Godunov (solveur de Riemann 2D exact), tout en gardant sa rapidité (cf. T.P. \#2). Remarquons qu'on aurait aussi bien pu appliquer la correction MUSCL au flux de Godunov en changeant simplement la dernière ligne de code au Listing 3. Les résultats obtenus avec le solveur de Riemann 2D exact sont donc a fortiori plus précis que ceux que nous présentons ici.}. Elles sont repérées dans le code ci-bas respectivement par $\verb|six|$, $\verb|siy|$, et $\verb|ri|$.
